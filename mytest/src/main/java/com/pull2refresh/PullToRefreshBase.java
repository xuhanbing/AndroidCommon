...
//						smoothScrollTo(value);
	...
	{
	    return createLoadingLayout(context, Mode.PULL_FROM_START, null);
	}
	protected LoadingLayout createFooterLoadingLayout(Context context)
	{
	    return createLoadingLayout(context, Mode.PULL_FROM_END, null);
	}
...
//		}
		...
		mHeaderLayout = createHeaderLoadingLayout(context);
		mFooterLayout = createFooterLoadingLayout(context);
...
//		a.recycle();
	...
	private boolean isReadyForPull() {
	    ...
	    /**
	     * we make it can be pulled while refreshing.
	     */
	    if (isRefreshing())
		return true;
		switch (mMode) {
	...
	private void pullEvent() {
		...
		/**
		 * if it is refreshing, then we cut content size 
		 */
		if (isRefreshing())
