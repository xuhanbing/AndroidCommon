...
public class StrengthScrollView extends ScrollView {
	...
	if (null != mChild) {
		...
		if (null != mScroller && !mScroller.isFinished()) {
			...
			mOnPullListener.onMoveBack(0, 0, mMaxPadding);
		    }
		    ;
     ...
     * 控制移动的最大值
     */
    protected void checkPullY() {
	int y = (int) Math.min(Math.abs(mPullY), mMaxPadding);
	if (0 != mPullY) {
	...
	if (null != mOnPullListener) {
	    mOnPullListener.onPull(dy, mPullY, mMaxPadding);
	}
    }
	...
	if (null != mOnPullListener) {
	    mOnPullListener.onMoveBack(0, y, mMaxPadding);
	}
    }
