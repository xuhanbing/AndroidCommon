...
public abstract class SDSQLiteOpenHelper {
     ...
     * from {@link android.content.ContentProdvider#onCreate ContentProvider.onCreate()}.
     *
...
//        	return getOnlyReadDatabase();
        }
    }
    
    private SQLiteDatabase getOnlyReadDatabase()
    {
    	if (mDatabaseRead != null) {
            if (!mDatabaseRead.isOpen()) {
                // Darn!  The user closed the database by calling mDatabase.close().
            	mDatabaseRead = null;
            } else if (mDatabaseRead.isReadOnly()) {
                // The database is already open for business.
                return mDatabaseRead;
            }
        }
        ...
        SQLiteDatabase db = null;//mDatabase;
        try {
...
//                }
                ...
                try {
                	final String path = getDatabasePath(mName).getPath();//mContext.getDatabasePath(mName).getPath();
...
//                                mFactory, mErrorHandler);
                    	final String path = getDatabasePath(mName).getPath();//mContext.getDatabasePath(mName).getPath();
                        db = SQLiteDatabase.openDatabase(path, mFactory,
                                SQLiteDatabase.OPEN_READWRITE|SQLiteDatabase.CREATE_IF_NECESSARY, mErrorHandler);
                    }
                } catch (SQLiteException ex) {
                    if (writable) {
                        throw ex;
    ...
    { 
        ...
        { 
            EXTERN_PATH = android.os.Environment.getExternalStorageDirectory().getAbsolutePath() + DEFAULT_DIR; 
